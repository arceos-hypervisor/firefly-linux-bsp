				  ...
				  u32 flags)
{
	...
	rq = task_rq_lock(p, &rf);
	return __set_cpus_allowed_ptr_locked(p, new_mask, rq, &rf, flags);
}
				     ...
				     const struct cpumask *subset_mask)
{
	...
	return __set_cpus_allowed_ptr_locked(p, new_mask, rq, &rf, 0);
}
...
int sched_cpu_drain_rq(unsigned int cpu)
{
	...
	return stop_one_cpu_async(cpu, __balance_push_cpu_stop, NULL, rq_drain,
				  rq_drain_done);
}
